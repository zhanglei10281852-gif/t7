...
const AlarmCenter: React.FC = () => {
  ...
  const getSeverityTag = (severity: string) => {
    ...
    return <Tag color={map[severity]?.color || 'default'}>{map[severity]?.text || severity}</Tag>;
  };
  ...
  const columns = [
    {
      ...
      onFilter: (value: string, record: Alarm) => record.severity === value,
    },
    ...
    },
    {
      ...
      render: (_: any, record: Alarm) =>
        record.status !== '已处理' ? (
          <Button type="link" icon={<CheckCircleOutlined />} onClick={() => handleHandle(record)}>
            处理
          </Button>
        ) : null,
    },
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={3} style={{ margin: 0 }}>报警中心</Title>
        <Space>
          <Select
            style={{ width: 200 }}
            placeholder="选择站点"
      ...
      <Card>
        {loading ? (
          ...
          <Table columns={columns} dataSource={alarms} rowKey="id" pagination={{ pageSize: 10 }} />
        )}
      </Card>
      ...
      >
        {selectedAlarm && (
          <div style={{ marginBottom: 16 }}>
            <p><strong>报警标题:</strong> {selectedAlarm.title}</p>
            <p><strong>报警类型:</strong> {selectedAlarm.alarm_type}</p>
            <p><strong>描述:</strong> {selectedAlarm.description}</p>
          </div>
        )}
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item name="status" label="处理状态" initialValue="处理中" rules={[{ required: true }]}>
            <Select>
              <Option value="处理中">处理中</Option>
              <Option value="已处理">已处理</Option>
            </Select>
          ...
          <Form.Item name="handled_by" label="处理人" rules={[{ required: true }]}>
